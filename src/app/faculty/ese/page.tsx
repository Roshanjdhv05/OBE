...
import { calculateESEStudentPercentage, evaluateESESkillAttainment, calculateESECOAttainment } from '@/lib/calculations/ese';
...
export default function ESEPage() {
  ...
  const loadStoredSummary = async (subId: string) => {
    ...
    const [config, assessments] = await Promise.all([
      ...
      OBEStore.getAssessments(subId, 'ESE')
    ]);
    ...
    if (cos.length > 0 && results.length > 0) {
      const summary = cos.map((co) => {
        ...
        const yesPct = calculateESECOAttainment(attainedCount, totalStudents);
        const noPct = Number(((noCount / Math.max(totalStudents, 1)) * 100).toFixed(2));
        const maxMarks = coResults[0]?.maximumMarks || 15;
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      const summary = preview.detectedCOs.map((coCode) => {
        ...
        const studentResults = (preview.studentMarks || [])
          .map((std) => {
            ...
            const pct = calculateESEStudentPercentage(marks.obtained, marks.max);
            return { attained: evaluateESESkillAttainment(pct, config.studentTargetPercentage) };
          })
        ...
        const yesPct = calculateESECOAttainment(attainedCount, totalStudents);
        const noPct = Number(((noCount / Math.max(totalStudents, 1)) * 100).toFixed(2));
        const targetMarks = Number(((config.studentTargetPercentage / 100) * (maxMarks || 15)).toFixed(1));
        const level = evaluateAttainmentLevel(yesPct, {
          level1: config.level1Threshold,
          level2: config.level2Threshold,
          level3: config.level3Threshold,
      ...
      console.error('Failed to import ESE assessment:', err);
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar
          ...
          subtitle="Upload ESE Excel spreadsheet to calculate student CO attainment"
          role="faculty"
        />
        ...
        <main className="p-8 space-y-6 max-w-5xl mx-auto w-full">
          ...
          {selectedSubId ? (
            ...
            <EmptyState
              title="Select a Subject to Upload ESE Assessment"
              description="Please select the Programme, Semester, and Subject context above."
              icon={FileCheck}
            />
          )}
